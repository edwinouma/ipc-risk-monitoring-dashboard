...
def main():
    ...
    for country in df["country"].unique():
        ...
        for ind in country_indicators:
            ...
            for baseline_method in baseline_list:
                    ...
                    try:
                        ...
                        if ind in PRICE_INDICATORS:
                            ...
                            df_z_source = df_rainfall_standard[
                                ...
                                (df_rainfall_standard[INDICATOR_COL] == ind)
                                ].copy()
                ...
                print("\n" + "=" * 80)
                print(f"CHECKING VCI : {country} | {ind}")
                print("=" * 80)

                if ind in VCI_INDICATORS:

                    print(f"✅ {ind} found in VCI_INDICATORS")
                    ...
                    try:
                        ...
                        print(f"Source rows: {len(df_vci_source)}")
                        print(df_vci_source.head())
                        ...
                        print(f"VCI rows returned: {len(df_vci)}")
                        print(df_vci.head())

                        # -----------------------------------------
                        # Export VCI output (QA / Debug)
                        # -----------------------------------------

                        df_vci_export = df_vci.copy()

                        if "year_month" in df_vci_export.columns:
                            df_vci_export["year_month"] = (
                                df_vci_export["year_month"]
                                .astype(str)
                            )

                        output_file = (
                            f"outputs/vci_output_{country}_{ind}.xlsx"
                            .replace(" ", "_")
                        )

                        df_vci_export.to_excel(
                            output_file,
                            index=False
                        )

                        print(f"VCI output saved to {output_file}")
                        ...
                        print(f"✅ VCI applied for {ind}")

                    except Exception as e:

                        print(f"VCI failed for {ind}: {e}")
                ...
                if method_type in ["event", "event_combined"]:
                    ...
                    # -----------------------------------------
                    if (
                            ind in VCI_INDICATORS
                            and "vci" in df_indicator.columns
                    ):
                        value_col_unit = "vci"

                    elif (
                            ind in ZSCORE_TRUE_INDICATORS
                            and "value_zscore" in df_indicator.columns
                    ):
                        value_col_unit = "value_zscore"
    ...
    for ind in INDICATORS:
        ...
        # -----------------------------------------
        # 🔥 SELECT VALUE COLUMN (TRIGGER)
        # -----------------------------------------

        if (
                ind in VCI_INDICATORS
                and "vci" in final_unit_month.columns
                and final_unit_month["vci"].notna().any()
        ):
    ...
    for ind in INDICATORS:
        ...
        # -----------------------------------------
        # SELECT VALUE COLUMN FOR CLASSIFICATION
        # -----------------------------------------

        if (
                ind in VCI_INDICATORS
                and "vci" in df_matrix.columns
                and df_matrix["vci"].notna().any()
        ):
